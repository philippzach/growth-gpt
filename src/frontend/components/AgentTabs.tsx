...
const AGENTS = [
  ...
  { id: 'persona-strategist', name: 'Persona', icon: '🧠', fullName: 'Persona Strategist' },
  { id: 'product-manager', name: 'PMF', icon: '📱', fullName: 'Product Manager' },
  { id: 'growth-manager', name: 'Growth', icon: '📈', fullName: 'Growth Manager' },
  { id: 'head-of-acquisition', name: 'Acquisition', icon: '🎣', fullName: 'Head of Acquisition' },
  { id: 'head-of-retention', name: 'Retention', icon: '🔄', fullName: 'Head of Retention' },
  { id: 'viral-growth-architect', name: 'Viral', icon: '🚀', fullName: 'Viral Growth Architect' },
  { id: 'growth-hacker', name: 'Hack', icon: '🧪', fullName: 'Growth Hacker' }
];

export default function AgentTabs({ session, currentAgent, viewingAgent, onAgentClick }: AgentTabsProps) {
  ...
  const getStatusStyles = (status: string) => {
    switch (status) {
      ...
      case 'current':
        return {
          container: 'bg-indigo-100 dark:bg-indigo-900/30 border-indigo-300 dark:border-indigo-700 text-indigo-800 dark:text-indigo-300',
      ...
      <div className="w-full">
        <div className="grid grid-cols-8 gap-0">
          {AGENTS.map((agent) => {
            ...
            const isClickable = onAgentClick && (status !== 'pending' || agent.id === currentAgent);
            ...
            return (
              ...
              >
                {/* Status indicator */}
                <div className={`absolute top-2 right-2 w-2 h-2 rounded-full ${styles.indicator}`} />
